...
class MPU6050:
    def __init__(self, bus=_E, freq=_E, sda=_E, scl=_E, addr=_MPU6050_ADDRESS):
        ...
        self.addr = addr
        try:
            self.i2c.writeto_mem(self.addr, _PWR_MGMT_1, bytes([0]))
            sleep_ms(5)
        except Exception as e:
            print(i2c_err_str.format(self.addr))
            raise e
        self._accel_range = self.get_accel_range(_F)
        self._gyro_range = self.get_gyro_range(_F)
        ...
        while failCount < _maxFails:
            try:
                ...
                if failCount >= _maxFails:
                    ...
                    print(i2c_err_str.format(self.addr))
                    return {_A: float(_G), _B: float(_G), _C: float(_G)}
    ...
    def set_accel_range(self, accel_range): self.i2c.writeto_mem(
        self.addr, _ACCEL_CONFIG, bytes([accel_range])); self._accel_range = accel_range
    ...
    def read_accel_data(self, g=_D):
        ...
        if accel_range == _ACC_RNG_2G:
            ...
            scaler = _ACC_SCLR_2G
        x = accel_data[_A]/scaler
        y = accel_data[_B]/scaler
        z = accel_data[_C]/scaler
        if g is _F:
            return {_A: x, _B: y, _C: z}
        elif g is _D:
            x = x*_GRAVITIY_MS2
    ...
    def read_accel_abs(self, g=_D): d = self.read_accel_data(
        g); return sqrt(d[_A]**2+d[_B]**2+d[_C]**2)

    def set_gyro_range(self, gyro_range): self.i2c.writeto_mem(
        self.addr, _GYRO_CONFIG, bytes([gyro_range])); self._gyro_range = gyro_range
    ...
    def read_gyro_data(self):
        ...
        if gyro_range == _GYR_RNG_250DEG:
            ...
            scaler = _GYR_SCLR_250DEG
        x = gyro_data[_A]/scaler
        y = gyro_data[_B]/scaler
        z = gyro_data[_C]/scaler
        return {_A: x, _B: y, _C: z}

    def read_angle(self): a = self.read_accel_data(); x = atan2(
        a[_B], a[_C]); y = atan2(-a[_A], a[_C]); return {_A: x, _B: y}
